...
void readInput(string requests_path, string workers_path) {
   ...
   cout << "begin tree" <<endl;
	...
	//Do this if you have not stored your time-dependent road network (tgraph_path) data in a TD-G-Tree file and want to build the TD-G-Tree while running the code.
	/*
		tree.buildTree();
	*/
	...
	//Extra detour time that customer is comfortable waiting for in Ridesharing scenarios. This value can be tweaked to see the impact of extra deadline time on requests served and execution time.
	double req_ddl = 600.0;

	clock_t tbegin = clock();

	for (int i = 0; i < n; ++ i) {

	//####Ridesharing Deadline####
	
	/* 
	ifs >> R[i].tim >> R[i].s >> R[i].e >> R[i].com >> R[i].sp;


	R[i].len = tree.tdsp(R[i].s,R[i].e,0.0,false);  //shortest path of the route from the request's pickup to request's dropoff location
	R[i].ddl= R[i].tim + R[i].sp + req_ddl;

	cout <<"R: "<<R[i].s<<", "<<R[i].e<<endl;
		cout <<"length of request: "<<R[i].ddl <<endl;
		
	*/

	//####Logistics Deadline####
	
		ifs >> R[i].tim >> R[i].s >> R[i].e >> R[i].com >>R[i].ddl >> R[i].sp;
		
	//	R[i].len = tree.tdsp(R[i].s,R[i].e,R[i].tim,false);
		R[i].ddl+= R[i].tim + R[i].sp;
		

		
	}
	clock_t tend = clock();
	cout << "Finish tdsps, \t time cost: " + to_string((tend - tbegin) / CLOCKS_PER_SEC) + "s" << endl;
...
void timeDependentInsertion(){
	...
	cout <<"num of queries: "<<numOfTDSP<<"tdspInitial: "<<numofTDSPInitial<<endl;
	cout << "car:" <<carnum << "  car time:" << cartime << "\n";
	cout << "assign time: "<<assigntime << "  num of assigns:" << assignnum << "\n";
	cout << "insertion time: "<< insertiontime << "  num of insertions:" << insertionnum << "\n";
	cout << "Requests served: "<< serverPassenger << "\n";
	cout <<"Total distance covered:" <<total_actual << endl;
...
void try_insertion(Worker &w, int rid, double &delta, int &optimanl_i, int &optimanl_j) {
	...
	for (int j = 0; j <= schedule.size(); j++)
	{
		...
		if (j == schedule.size())
		{
			...
			//modified code to show the tdsp values:
		}
		else
		{
			if (tmp <= r.ddl)
...
void insertion(Worker &w, int rid, int wid, int optimal_i, int optimal_j){
    ...
    cout << "Assign request:" << rid <<" with deadline: "<< R[rid].ddl << " to worker : " << wid << endl;
    cout << "before insertion: "<<endl;
    cout <<  w.S << endl;
    cout <<  w.reach << endl;
